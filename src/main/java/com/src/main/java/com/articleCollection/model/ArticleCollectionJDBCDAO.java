...
public class ArticleCollectionJDBCDAO implements ArticleCollectionDAO {
	...
	public static final String GET_ONE_STMT = "SELECT * FROM ARTICLE_COLLECTION WHERE ARTICLE_ID = ?,MEM_ID = ?;";
	public static final String GET_ALL_STMT = "SELECT * FROM ARTICLE_COLLECTION;";
	...
	public void add(ArticleCollectionVO articleCollection) {
		...
		try {
			...
			pstmt.setDate(3, articleCollection.getArticle_follow_date());
...
//	}
	...
	@Override
	public void delete(int article_ID, int mem_ID) {
		Connection con = null;
		PreparedStatement pstmt = null;
	...
	@Override
	public ArticleCollectionVO findByPK(int article_ID, int mem_ID) {
		...
		try {
			...
			while (rs.next()) {
				...
				article_collection.setArticle_follow_date(rs.getDate("ARTICLE_FOLLOW_DATE"));

...
public class ArticleReportJDBCDAO implements ArticleReportDAO {
	...
	public static final String UPDATE_STMT = "UPDATE ARTICLE_REPORT SET REPORT_STATE=?, REPORT_NOTE=?, REPORT_END_TIME= ? WHERE ARTICLE_REPORT_ID = ?;";
// public static final String DELETE_STMT = "DELETE FROM ARTICLE_REPORT WHERE
	...
	public void update(ArticleReportVO articleReport) {
		...
		try {
			...
			pstmt.setDate(3, articleReport.getReport_end_time());
			pstmt.setInt(4, articleReport.getArticle_report_ID());
...
//public void delete(Integer report_ID) {}
	@Override
	public ArticleReportVO findByPK(int article_report_ID) {
		...
		try {
			...
			while (rs.next()) {
				...
				articleReport.setReport_time(rs.getDate("REPORT_TIME"));
				...
				articleReport.setReport_end_time(rs.getDate("REPORT_END_TIME"));
			}
		} catch (SQLException se) {
			se.printStackTrace();
		} finally {

        ...
        if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
            ...
            try {
                ...
                if (!errorMsgs.isEmpty()) {
                    RequestDispatcher failureView = req.getRequestDispatcher("/front-end/projOrder/searchOrder.jsp");///***我還沒改
                ...
                RequestDispatcher failureView = req.getRequestDispatcher("/front-end/projOrder/searchOrder.jsp");
                failureView.forward(req, res);
            }
        }
...
//					System.out.println(str1);
                ...
                // Send the use back to the form, if there were errors
                if (!errorMsgs.isEmpty()) {
                    RequestDispatcher failureView = req.getRequestDispatcher("/front-end/projOrder/listAllOrderByFmem.jsp");///***我還沒改
                    failureView.forward(req, res);
                    return;//程式中斷
                }
...
//				System.out.println("error in final");
        ...
        if ("getOne_For_Update_ByMem".equals(action)) { // 來自listAllEmp.jsp的請求
            ...
            try {
                ...
                RequestDispatcher failureView = req.getRequestDispatcher("/front-end/projOrder/listAllOrderByMem.jsp");///***我還沒改
                failureView.forward(req, res);
            }
        }
...
//		req.setAttribute("projOrderVO", projOrderVO); // 含有輸入格式錯誤的empVO物件,也存入req
                    RequestDispatcher failureView = req.getRequestDispatcher("/front-end/projOrder/update_order_input_ByMem.jsp");
                    failureView.forward(req, res);
                    return; //程式中斷
                }
...
//				res.sendRedirect("listAllOrderByFmem.jsp");
                ...
                errorMsgs.add("修改資料失敗:" + e.getMessage());
                e.printStackTrace();
                RequestDispatcher failureView = req.getRequestDispatcher("/front-end/home/home.jsp");
                failureView.forward(req, res);
            }
        }
        /*************我增加的部分，按按鈕改變訂單狀態變成 1 待出貨，END  *****************/
...
//				res.sendRedirect("/front-end/projOrder/listAllOrderByFmem.jsp");
        ...
        if ("insert".equals(action)) { // 新增訂單的請求

...
<%@ page import="com.farmTravelCollection.model.*"%>
<%
    ...
    session.setAttribute("mem_ID",77000);
%>
<html>
    <head>
        <meta charset='utf-8'>
    ...
    <body>
                        ...
                        <tr>
                            <th scope='row'>${farmTravel.farm_travel_ID}<br/>
                                <button type='button' class='btn btn-outline-light' value='${farmTravel.farm_travel_ID}'>
                                    <img src='<%=request.getContextPath()%>/farmTravelCollection.do?action=collection&mem_ID=${sessionScope.mem_ID}&farm_travel_ID=${farmTravel.farm_travel_ID}' class='collection'>
                                </button>
                            </th>
                            ...
                            <td>
                                <c:if test="${farmTravel.farm_travel_state eq 0 && farmTravel.farm_travel_now eq 0}">
                                    <button type='button' class='update btn btn-outline-secondary' value='${farmTravel.farm_travel_ID}'>修改</button>
                                </c:if>
                                <c:if test="${farmTravel.farm_travel_state eq 0 && farmTravel.farm_travel_now eq 0}">
                                    <button type='button' class='delete btn btn-outline-danger' value='${farmTravel.farm_travel_ID}'>刪除</button>
                                </c:if>
                            </td>
                            <td> <button type='button' class='getOne btn btn-outline-primary' value='${farmTravel.farm_travel_ID}'>查看詳情</button></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
        ...
        <script>
            ...
            $('.update').click(function (){
                $.post(
                    '<%=request.getContextPath()%>/farmTravel.do',
                    { action:'getOne', farm_travel_ID:$(this).val() },
                    ()=>{
                        window.location.href = '<%=request.getContextPath()%>/front-end/farmTravel/updateFarmTravel.jsp';
                    }
                );
            });
            $('.getOne').click(function (){
                $.post(
                    ...
                    { action:'getOne', farm_travel_ID:$(this).val() },
                    ()=>{
                        window.location.href = '<%=request.getContextPath()%>/front-end/farmTravel/listOneFarmTravel.jsp';
                    }
                );
            });
        </script>

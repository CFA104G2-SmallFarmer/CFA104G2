...
<body>
  ...
  <main class="main">
    <h1 class="title1">修改幻燈片</h1>
	<h2 class="title2">管理員編號:<%=adminLoginVO.getAdmin_id()%></h2>
 	<h2 class="title2">幻燈片編號:<%=homeSlideShowVO.getHss_id()%></h2>
...
<%--         value="<%=homeSlideShowVO.getHss_state() %>" /> --%>
<!--       </div> -->

      <div class="item_block">
        <label>圖片描述:</label>
        <input type="TEXT" name="hss_text" size="45"
        value="<%=homeSlideShowVO.getHss_text()%>" />
      </div>
   ...
   <input type="hidden" name="hss_id" value="<%=homeSlideShowVO.getHss_id()%>">
...
<script>
        ...
        $('#f_date1').datetimepicker({
         ...
         step: 1,                
         format:'Y-m-d',         
       value: '<%=homeSlideShowVO.getHss_time()%>'
  });
        $('#f_date2').datetimepicker({
           ...
           step: 1,                
           format:'Y-m-d',         
         value: '<%=homeSlideShowVO.getHss_on_shelf()%>'
  });
        $('#f_date3').datetimepicker({
           ...
           step: 1,                
           format:'Y-m-d',         
         value: '<%=homeSlideShowVO.getHss_off_shelf()%>'
  });
</script>
   </html>
